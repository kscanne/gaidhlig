...
open(STEMS, "<:utf8", 'juststem-gd.txt') or die "Could not open stem list: $!";
while (<STEMS>) {
	chomp;
	my $line = $_;
...
open(FREQ, "<:utf8", '/home/kps/seal/idirlamha/gd/freq/freq.txt') or die "Could not open freq list: $!";
while (<FREQ>) {
	chomp;
	my $line = $_;
